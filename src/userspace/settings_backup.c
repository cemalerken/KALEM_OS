...
// Demo ekran çözünürlükleri
static display_resolution_t demo_resolutions[] = {
    {640, 480, 16, "640x480 (16-bit)"},
    {800, 600, 16, "800x600 (16-bit)"},
    {1024, 768, 16, "1024x768 (16-bit)"},
    {1280, 720, 16, "1280x720 (16-bit)"},
    {1366, 768, 16, "1366x768 (16-bit)"},
    {1600, 900, 16, "1600x900 (16-bit)"},
    {1920, 1080, 16, "1920x1080 (16-bit)"}
};

// Demo ağ adaptörleri
static network_adapter_t demo_adapters[] = {
    {"Realtek RTL8111/8168 Ethernet", "00:1A:2B:3C:4D:5E", 0, 1},
    {"Intel Wireless-AC 8265", "A1:B2:C3:D4:E5:F6", 1, 1}
};
...
void settings_init() {
    ...
    if (active_settings == NULL) {
        ...
        // Demo donanım bilgilerini doldur
        // Ekran kartı
        strcpy(active_settings->display.name, "Intel HD Graphics 630");
        active_settings->display.vram_size = 128 * 1024 * 1024; // 128 MB
        active_settings->display.resolution_count = sizeof(demo_resolutions) / sizeof(display_resolution_t);
        active_settings->display.resolutions = demo_resolutions;
        active_settings->display.current_resolution = 2; // 1024x768
        
        // Ses kartı
        strcpy(active_settings->sound.name, "Realtek ALC1200 Audio");
        active_settings->sound.channels = 2;
        active_settings->sound.volume = 80;
        active_settings->sound.muted = 0;
        
        // Ağ adaptörleri
        active_settings->network_adapter_count = sizeof(demo_adapters) / sizeof(network_adapter_t);
        active_settings->network_adapters = demo_adapters;
        
        // Sistem ayarları
        strcpy(active_settings->system.hostname, "KALEM-PC");
        strcpy(active_settings->system.username, "Kullanıcı");
        active_settings->system.theme = 0; // Varsayılan tema
        active_settings->system.autostart_gui = 1;
        active_settings->system.screen_saver_timeout = 10; // 10 dakika
        active_settings->system.power_save_mode = 1; // Düşük
        
        // Tarih/saat
        active_settings->datetime.year = 2023;
        active_settings->datetime.month = 10;
        active_settings->datetime.day = 15;
        active_settings->datetime.hour = 14;
        active_settings->datetime.minute = 30;
        active_settings->datetime.second = 0;
        active_settings->datetime.timezone = 3; // Türkiye (UTC+3)
        
        // Dil
        active_settings->language = 0; // Türkçe
        
        // Sürücü durumlarını başlat ve otomatik tanı
        driver_manager.current_display_driver = settings_detect_display_driver();
        driver_manager.current_sound_driver = settings_detect_sound_driver();
        
        // Sürücüleri hemen kur
        if (driver_manager.current_display_driver) {
            driver_manager.current_display_driver->installed = 1;
        }
        
        if (driver_manager.current_sound_driver) {
            driver_manager.current_sound_driver->installed = 1;
        }
        
        // Depolama yönetimini başlat
        settings_detect_storage();
        
        // USB monitor'u başlat
        settings_usb_monitor_init();
        
        // Yapay zeka destekli otomatik sürücü kurulumu aktifse çalıştır
        if (usb_monitor.ai_assist_enabled) {
            settings_ai_assisted_driver_setup();
        }
...
void settings_show() {
    ...
    active_settings->window->on_close = [](gui_window_t* window) {
        if (active_settings) {
...
void settings_detect_hardware() {
    // Bu demo sürümünde sadece simüle edilmiş donanım bilgileri mevcut
...
void settings_detect_display() {
    // Demo ekran kartı bilgilerini doldur (zaten doldurulmuş)
}
...
void settings_detect_network() {
    // Demo ağ adaptörü bilgilerini doldur (zaten doldurulmuş)
}
...
int settings_change_resolution(uint32_t width, uint32_t height, uint8_t bpp) {
    ...
    for (uint32_t i = 0; i < active_settings->display.resolution_count; i++) {
        display_resolution_t* res = &active_settings->display.resolutions[i];
...
void settings_paint(gui_window_t* window) {
    ...
    // Kategorileri ve ayrıcıyı çiz
    settings_paint_categories(window);
    
    // Dikey ayırıcı
    gui_draw_rect(window, 150, 40, 2, window->height - 60, COLOR_DARK_GRAY);
    
    // Aktif kategoriyi çiz
...
void settings_paint_categories(gui_window_t* window) {
    ...
    for (int i = 0; i < SETTINGS_CAT_COUNT; i++) {
        ...
        int text_color = (i == active_settings->current_category) ? COLOR_BLACK : COLOR_DARK_GRAY;
        
        // Kategori arkaplanını çiz
        gui_draw_rect(window, 10, y, 140, 30, bg_color);
...
void settings_paint_display(gui_window_t* window) {
    ...
    // Başlık
    gui_draw_text(window, "Ekran Ayarları", 180, 50, COLOR_BLACK);
    
    // Ekran kartı bilgileri
    gui_draw_text(window, "Ekran Kartı:", 180, 90, COLOR_DARK_GRAY);
    gui_draw_text(window, active_settings->display.name, 300, 90, COLOR_BLACK);
    
    // VRAM bilgileri
    gui_draw_text(window, "Video Belleği:", 180, 125, COLOR_DARK_GRAY);
    char vram_text[32];
    sprintf(vram_text, "%d MB", active_settings->display.vram_size / (1024 * 1024));
    gui_draw_text(window, vram_text, 300, 125, COLOR_BLACK);
    
    // Çözünürlük
    gui_draw_text(window, "Çözünürlük:", 180, 160, COLOR_DARK_GRAY);
    ...
    for (uint32_t i = 0; i < active_settings->display.resolution_count; i++) {
        ...
        // Seçili mi?
        int bg_color = (i == active_settings->display.current_resolution) ? COLOR_LIGHT_BLUE : COLOR_LIGHT_GRAY;
...
void settings_paint_network(gui_window_t* window) {
    ...
    // Başlık
    gui_draw_text(window, "Ağ Ayarları", 180, 50, COLOR_BLACK);
    
    // Ağ adaptörlerini listele
    int y = 90;
    for (uint32_t i = 0; i < active_settings->network_adapter_count; i++) {
        network_adapter_t* adapter = &active_settings->network_adapters[i];
        
        // Adaptör adı
        gui_draw_text(window, adapter->name, 180, y, COLOR_BLACK);
        ...
        // Yatay çizgi
        gui_draw_rect(window, 180, y + 50, 380, 1, COLOR_DARK_GRAY);
        
        y += 70;
    }
    
    // Bağlantı yapılandırma butonu
...
void settings_paint_hardware(gui_window_t* window) {
    ...
    // Başlık
    gui_draw_text(window, "Donanım Yönetimi", 180, 50, COLOR_BLACK);
    gui_draw_text(window, "Yapay Zeka Destekli Otonom Tanıma", 400, 50, COLOR_DARK_GREEN);
    
    // İlk bölüm - Donanım tanıma
    gui_draw_text(window, "Donanım Tanıma:", 180, 85, COLOR_DARK_GRAY);
    ...
    if (driver_manager.display_driver) {
        ...
        gui_draw_text(window, "Ekran kartı algılanamadı.", 280, 130, COLOR_DARK_RED);
        gui_draw_button(window, 500, 130, 130, 30, "Sürücü Ara", COLOR_BLUE, COLOR_WHITE);
    }
    ...
    if (driver_manager.sound_driver) {
        ...
        gui_draw_text(window, "Ses kartı algılanamadı.", 280, 230, COLOR_DARK_RED);
        gui_draw_button(window, 500, 230, 130, 30, "Sürücü Ara", COLOR_BLUE, COLOR_WHITE);
    }
    
    // Yapay zeka durumu
    gui_draw_text(window, "Yapay Zeka Desteği:", 180, 320, COLOR_BLACK);
    ...
    gui_draw_text(window, "Otonom Sistem Ayarları:", 180, 400, COLOR_BLACK);
    
    gui_draw_text(window, "Otomatik Donanım Algılama:", 200, 430, COLOR_DARK_GRAY);
    gui_draw_checkbox(window, 370, 430, 1);  // Varsayılan olarak açık
    
    gui_draw_text(window, "Otomatik Sürücü Kurulumu:", 200, 460, COLOR_DARK_GRAY);
    gui_draw_checkbox(window, 370, 460, 1);  // Varsayılan olarak açık
    
    gui_draw_text(window, "Sürücü Güncellemelerini Kontrol Et:", 200, 490, COLOR_DARK_GRAY);
    gui_draw_checkbox(window, 370, 490, 1);  // Varsayılan olarak açık
}
